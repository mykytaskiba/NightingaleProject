...
public:
	virtual std::string const& getCommand() const = 0;
	virtual std::string getUsage() const = 0;
	virtual void execute(std::string args, ExecutionState& state, ExecutionResult& result) = 0;
};
...
public:
	...
	Command(std::string const& commandStr) : m_command(commandStr) {}

	std::string const& getCommand() const override {
		return m_command;
	}
	...
	virtual void execute(std::string args, ExecutionState& state, ExecutionResult& result) override {
